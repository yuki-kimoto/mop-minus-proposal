package mop::minus::internals::util;
use v5.16;
use warnings;
...
sub apply_all_roles {
    ...
    foreach my $method ($composite->methods) {
        if (my $existing_method = $to->get_method($method->name)) {
            mop::minus::apply_metaclass($existing_method, $method);
        }
        else {
    ...
    for my $conflict ($composite->required_methods) {
        if (my $method = $to->get_method($conflict)) {
            ...
            for my $conflicting_method (@conflicting_methods) {
                mop::minus::apply_metaclass($method, $conflicting_method);
            }
        }
        else {
...
# mop::minus::object::new.
sub find_or_inflate_meta {
    my ($class) = @_;

    if (my $meta = mop::minus::meta($class)) {
        return $meta;
    }
    else {
...
sub inflate_meta {
    ...
    my $new_meta = mop::minus::class->new(
        name       => $name,
        version    => $version,
        authority  => $authority,
        superclass => $isa->[0],
...
sub rebase_metaclasses {
    ...
    pop @meta_isa;
    @meta_isa = reverse map { mop::minus::meta($_) } @meta_isa;

    my @super_isa = @{ mro::get_linear_isa($super_name) };
    pop @super_isa until $super_isa[-1] eq $common_base;
    pop @super_isa;
    @super_isa = reverse map { mop::minus::meta($_) } @super_isa;
    ...
    for my $class (@meta_isa) {
        ...
        my $rebased = "mop::minus::class::rebased::${class_name}::for::${current}";
        if (!mop::minus::meta($rebased)) {
...
sub create_composite_role {
    ...
    @roles = map { ref($_) ? $_ : mop::minus::meta($_) } @roles;

    return $roles[0] if @roles == 1;

    my $name = 'mop::minus::role::COMPOSITE::OF::'
             . (join '::' => map { $_->name } @roles);
    return mop::minus::meta($name) if mop::minus::meta($name);

    my $composite = mop::minus::role->new(
        name  => $name,
        roles => [ @roles ],
    );
...
mop::minus::internals::util - internal use only
